...
class SSTDataset(Dataset):
    ...
    def __init__(self, split="train", root=False, binary=True):
        ...
        if root and binary:
            self.data = [
                (
                    rpad(tokenizer.encode("[CLS] " + tree.to_lines()[0] + " [SEP]"), n=66), get_binary_label(tree.label),
                )
            ...
            self.data = [
                (
                    rpad(tokenizer.encode("[CLS] " + tree.to_lines()[0] + " [SEP]"), n=66),tree.label,
                 )
                for tree in self.sst
            ]
            ...
            self.data = [
                (
                    rpad(tokenizer.encode("[CLS] " + line + " [SEP]"), n=66), tree.label,
                 )
                for tree in self.sst
            ...
            self.data = [
                (
                    rpad(tokenizer.encode("[CLS] " + line + " [SEP]"), n=66),get_binary_label(label),
                )
                for tree in self.sst
                for label, line in tree.to_labeled_lines()
                if label != 2
